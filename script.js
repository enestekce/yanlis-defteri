...
let gorevler = JSON.parse(localStorage.getItem("gorevler_" + aktifKullanici)) || [];
liste.innerHTML="";

if(gorevler.length===0){
    liste.innerHTML = "<p>Henüz görev eklenmedi 🎯</p>";
    return;
}
...
let yuzde = gorev.tamamlandi ? 100 : 0;

liste.innerHTML += `
<div class="box gorevKart ${gorev.tamamlandi ? 'tamamlandi':''}">
<h3>📚 ${gorev.ders} ${gorev.tamamlandi ? '✅':''}</h3>
<p>${gorev.tur}</p>
<p>🎯 Hedef: ${gorev.hedef}</p>
<p>Durum: ${gorev.tamamlandi ? 'Tamamlandı' : 'Devam ediyor'}</p>
<div class="progress"><div style="width:${yuzde}%"></div></div>

<button onclick="gorevTamamla(${index})">
${gorev.tamamlandi ? '↩ Geri al' : '✅ Tamamla'}
</button>

<button onclick="gorevSil(${index})">🗑️ Sil</button>

${gorev.link ? `<a href="${gorev.link}" target="_blank">▶️ Video</a>` : ''}
</div>`;
...
localStorage.setItem("gorevler_" + aktifKullanici, JSON.stringify(gorevler));
gorevleriGoster();
}
